...
# Public Route Table
resource "aws_route_table" "route-table-public-subnet-2" {
  vpc_id = aws_vpc.my-vpc.id
  route {
    cidr_block = "0.0.0.0/0"
    gateway_id = aws_internet_gateway.internet-gateway.id
...
resource "aws_route_table_association" "public-subnet-2-route-table-association" {
  ...
  route_table_id = aws_route_table.route-table-public-subnet-2.id
}

# Elastic IP for Nat-Gateway
resource "aws_eip" "elastic-for-my-nat-gateway-2" {
  vpc = true
  tags = {
    "Name" = "elastic-ip-2"
  }
...
resource "aws_nat_gateway" "my-nat-gateway-2" {
  allocation_id = aws_eip.elastic-for-my-nat-gateway-2.id
  subnet_id = aws_subnet.public-subnet-2.id
  tags = {
